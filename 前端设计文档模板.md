...
| AI训练看板 | /dashboard/training | AI训练看板页面 | 高 |
| AI训练看板详情 | /dashboard/training/detail/:id | AI训练看板详情页 | 高 |
...
#### 4.2.1 首页
- **路由路径**：`/`
- **页面描述**：应用首页，包含四个导航页签，用户可以通过页签跳转到对应的看板页面
- **功能需求**：
  - 显示四个导航页签：组织/岗位AI成熟度看板、AI训练看板、AI School看板、AI任职认证看板
  - 支持页签切换，点击页签跳转到对应的看板页面
  - 页签支持路由导航，使用 Vue Router 进行页面跳转
  - 支持页签激活状态显示
- **UI设计**：
  - 使用 Element Plus 的 `el-tabs` 组件实现页签导航
  - 页签布局：横向排列，支持响应式设计
  - 激活页签高亮显示
- **交互逻辑**：
...
- **页面描述**：展示AI训练相关的数据看板，包含训练进度、训练任务等信息
- **功能需求**：
  - 展示AI训练任务列表
  - 显示训练进度和状态
  - 支持筛选和搜索功能
...
- **页面描述**：AI训练看板的详情页面，展示单个训练任务的详细信息
- **功能需求**：
  - 展示训练任务的详细信息
  - 显示训练日志和进度详情
  - 支持操作按钮（开始、暂停、停止等）
  - 支持返回列表页
- **UI设计**：
  - 使用 Element Plus 的卡片、描述列表、时间线等组件
  - 详情信息分区域展示
- **交互逻辑**：
  - 从AI训练看板列表页点击进入
  - 支持返回按钮返回列表页
...
- **页面描述**：AI School看板的详情页面，展示单个课程的详细信息
- **功能需求**：
  - 展示课程的详细信息
  - 显示课程章节和学习进度
  - 支持开始学习、继续学习等操作
  - 支持返回列表页
- **UI设计**：
  - 使用 Element Plus 的卡片、步骤条、进度条等组件
  - 课程信息分区域展示
- **交互逻辑**：
  - 从AI School看板列表页点击进入
  - 支持返回按钮返回列表页
...
- **页面描述**：展示AI任职认证相关的数据看板，包含认证考试、认证记录等信息
- **功能需求**：
  - 展示认证考试列表
  - 显示认证状态和通过率
  - 支持筛选和搜索功能
...
- **页面描述**：AI任职认证看板的详情页面，展示单个认证考试的详细信息
- **功能需求**：
  - 展示认证考试的详细信息
  - 显示考试题目和答案
  - 支持开始考试、查看成绩等操作
...
- **受保护路由**：
  ...
  - 包括：组织/岗位AI成熟度看板、AI训练看板、AI School看板、AI任职认证看板及其详情页
- **权限控制方案**：
